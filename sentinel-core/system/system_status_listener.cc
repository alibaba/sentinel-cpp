...
size_t SystemStatusListener::GetActiveTime(std::shared_ptr<CpuUsageInfo> p) {
  return  p->times[S_USER] + p->times[S_NICE] +
          p->times[S_SYSTEM] + p->times[S_IRQ] +
          p->times[S_SOFTIRQ] + p->times[S_STEAL] +
...
void SystemStatusListener::ReadCpuUsageFromProc(std::shared_ptr<CpuUsageInfo> p) {
  std::string line;
...
void SystemStatusListener::UpdateCpuUsage() {
  ...
  // 100ms pause
  size_t active_time = GetActiveTime(usage_info_p2_) - GetActiveTime(usage_info_p1_);
  size_t idle_time = (GetIdleTime(usage_info_p2_) - GetIdleTime(usage_info_p1_));
  size_t total_time = active_time + idle_time;
  cur_cpu_usage_ = 1.f * active_time / total_time;
}
...
void SystemStatusListener::UpdateCpuLoad(){
  ...
  file_load_.close();
  cur_load_ = load_info_p_->usage[0];
}
...
//     std::cout << status_listener.GetCurLoad() << " " 
//       << status_listener.GetCurCpuUsage() << std::endl;
